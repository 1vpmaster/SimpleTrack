    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        if self.data_is_zipped:
            if is_gt:
                ...
                num_timesteps = int(self.seq_lengths[seq]) # for half val
                current_time_keys = [str( t+ 301) for t in range(num_timesteps)]
            file = seq + '.txt'
        else:
            zip_file = None
            if is_gt:
                file = self.config["GT_LOC_FORMAT"].format(gt_folder=self.gt_fol, seq=seq)
                num_timesteps = int(self.seq_lengths[seq]/2)
                half = int(self.seq_lengths[seq]/2) + 1
            else:
                file = os.path.join(self.tracker_fol, tracker, self.tracker_sub_fol, seq + '.txt')
                num_timesteps = int(self.seq_lengths[seq]/2) # for half val
                half = int(self.seq_lengths[seq]/2) + 1
        ...
        current_time_keys = [str( t+ half) for t in range(num_timesteps)]
