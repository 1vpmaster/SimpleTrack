...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('task', default='mot', help='mot or MultiFrameMot')
    ...
    # model
    self.parser.add_argument('--arch', default='decouple_34', 
                             help='model architecture. Currently tested'
                                  'resdcn_34 | resdcn_50 | resfpndcn_34 |'
                                  'dla_34 | hrnet_18 | multidla_34 | multidlaorign_34 | csdla_34 | decouple_34')
    ...
    self.parser.add_argument('--lr', type=float, default=2.5e-5,
                             help='learning rate for batch size 6.')
    ...
    self.parser.add_argument('--trainval', action='store_true',
                             help='include validation in training and '
                                  'test on test set')
    # 时间帧特征聚合
    self.parser.add_argument('--memory_enable', type=bool, default=True,
                            help='default use memory frames')
    self.parser.add_argument('--local_frame_dist', type=int, default=10,
                            help='local frames range area, from cur frames to before')
    self.parser.add_argument('--local_frame_num', type=int, default=3,
                            help='random choice local numbers')
    self.parser.add_argument('--memory_frame_dist', type=int, default=20,
                            help='memory frames range area, from cur frame to before')
    self.parser.add_argument('--memory_frame_num', type=int, default=3,
                            help='random choice local numbers')
    ...
    self.parser.add_argument('--det_thres', type=float, default=0.3, help='confidence thresh for detection') # 0.3
    self.parser.add_argument('--nms_thres', type=float, default=0.4, help='iou thresh for nms') # 0.4
    self.parser.add_argument('--track_buffer', type=int, default=30, help='tracking buffer') #初始是30
    self.parser.add_argument('--min-box-area', type=float, default=100, help='filter out tiny boxes') # MOT20 最小是28
    ...
    self.parser.add_argument('--data_dir', type=str, default='/dataset/Mix')
    ...
    self.parser.add_argument('--wh_weight', type=float, default=0.1,
                             help='loss weight for bounding box size.') # GIOU 设为1
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'mot' or 'MultiFrameMot':
  ...
  def init(self, args=''):
    default_dataset_info = {
      ...
      'MultiFrameMot': {'default_resolution': [608, 1088], 'num_classes': 1,
                'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
                'dataset': 'jde', 'nID': 14455},
    }
    class Struct:
