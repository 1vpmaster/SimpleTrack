...
class STrack_me(BaseTrack):
    ...
    def update_features(self, feat):
        ...
        self.smooth_feat /= np.linalg.norm(self.smooth_feat) # 这里特征已经融合了前几帧的特征
        ...
        if len(stracks) > 0:
            ...
            for i, st in enumerate(stracks):
                if st.state != TrackState.Tracked:
                    multi_mean[i][7] = 0
            multi_mean, multi_covariance = STrack_me.shared_kalman.multi_predict(multi_mean, multi_covariance)
...
class EGTracker(object):
    def __init__(self, opt, frame_rate=30):
        ...
        self.det_thresh = opt.conf_thres + 0.3
    ...
    def update(self, im_blob, img0):
        ...
        if len(dets) > 0:
            '''Detections'''
            detections = [STrack_me(STrack_me.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4],f, 30) for
        ...
        STrack_me.multi_predict(strack_pool)
        ...
        dists_fuse = dists_embedding + dists_giou

        # if dists_giou.size!=0:
        #     np.savetxt('dists_embedding.txt',dists_embedding,fmt='%0.3f')
        #     np.savetxt('dists_giou.txt',dists_giou,fmt='%0.3f')
        #     np.savetxt('dists_fuse.txt',dists_fuse,fmt='%0.3f')
        #     np.savetxt('dists_orign_fuse.txt',dists_orign_fuse,fmt='%0.3f')
        #     # plt.rc('font',family='Times New Roman')
        #     title_name = ["dists_giou", "dists_embedding", "dists_orign_fuse","dists_fuse"]
            
        #     fig, axs = plt.subplots(nrows=2, ncols=2)
        #     for ax, title in zip(axs.flat, title_name):
        #         # ax.imshow(eval(title))
        #         plt.tight_layout()
        #         im = ax.matshow(eval(title))
        #         fig.colorbar(im, shrink=0.5, ax=ax)
        #         ax.set_title(title)
                
        #     plt.savefig("dists.jpg")
        #     plt.close()
        ...
        if len(dets_second) > 0:
            '''Detections'''
            detections_second = [STrack_me(STrack_me.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4], f, 30) for
        ...
        for it in u_track:
            ...
            if 0 < track_index < meta['out_width']*meta['out_height']:
                ...
                if dists_reback[0] < 0.1:
                    det = STrack_me(STrack_me.tlbr_to_tlwh(track.tlbr), track.score, reback_feature[0,:], 2) 
                    if track.state == TrackState.Tracked:
                        track.update(det, self.frame_id)
                        activated_starcks.append(track)
                    else:
        ...
        self.tracked_stracks = remove_fp_stracks(self.tracked_stracks) # for track reback
