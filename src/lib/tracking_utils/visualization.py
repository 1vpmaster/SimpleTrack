...
def plot_tracking(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., ids2=None):
    ...
    for i, tlwh in enumerate(tlwhs):
        ...
        # obj_score = round(scores[i],2)
        # id_text = '{}-{}'.format(int(obj_id), str(obj_score))
...
def plot_trajectory(image, tlwhs, track_ids):
    image = image.copy()
    for i, (one_tlwhs, track_id) in enumerate(zip(tlwhs, track_ids)):
        if i<len(tlwhs)-1:
            ...
            for tlwh, id in zip(one_tlwhs,track_id):
                x1, y1, w, h = tuple(map(int, tlwh))
                color = get_color(int(id))
                cv2.circle(image, (int(x1 + 0.5 * w), int(y1 + h)), 2, (0,0,0), thickness=4)
