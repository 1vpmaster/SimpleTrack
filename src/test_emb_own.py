...
def plot_embeddings(embeddings, id_labels):
    ...
    plot_exp_name = "embeddings.png"
    plot_path = os.path.join("/home/tmp/MOT_myself/results",plot_exp_name)
        ...
        if batch_i % print_interval == 0:
            print(
                'Extracting {}/{}, # of instances {}, time {:.2f} sec.'.format(batch_i, len(dataloader), len(id_labels),
                                                                               time.time() - t))
            embedding_plot = torch.stack(embedding, dim=0).cuda()
            id_labels_plot = torch.LongTensor(id_labels)
            plot_embeddings(embedding_plot, id_labels_plot)
